...
- **Exact zones** — select the pixels each synthesizer plays by drawing rectangles or free-hand lassos directly on the image. A selection is stored as exact connected components of cells (run-length encoded): a rectangle or lasso shape touching an existing zone fuses with it — contiguous pixels are one zone — and an erase in the middle of a zone splits it into fragments that keep their creation order (the "zone by zone" reading order). A rectangle dragged from a free pixel adds cells, while one dragged from an already selected pixel removes them instead — a simple click selects or deselects a single pixel. The zone row displays the exact number of selected pixels. Zones can be edited while the synth is playing: the playhead stays on the pixel it is playing (it is remapped into the new selection), and the zone under the playhead is locked against erasure — an erasing drag touching it is cancelled. Emptying the zones stops the synth cleanly.
