...
- Adjusting the number of columns with a logarithmic-scale slider (the height is deduced automatically to preserve the aspect ratio). The change applies when the slider is released: every synth's zones are remapped run by run — each horizontal run of cells keeps its length while keeping its relative position in the image, and runs that would stick out are clamped inside the grid — then recomposed: zones that touch after the move fuse permanently (exact union, never an over-covering bounding box), and each playhead stays on the same pixel. The behavior is identical whether synths are playing or not. On the vertical axis a shrinking grid packs several source rows onto one target row, so complex shapes compress vertically.
